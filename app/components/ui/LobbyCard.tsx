...
export default function LobbyCard({ id, name, players, variant, onClick, onCreated }: lobbyProps) {
    ...
    return (
        variant == "create" ? 
            <div onClick={onClick} className="text-accent bg-secondary border-3 border-accent hover:border-accent/70 transition duration-100 hover:shadow-lg hover:shadow-accent/20 rounded-2xl h-60 sm:h-75 aspect-square flex flex-col justify-center items-center">
            ...
            <button onClick={onClick} className="text-accent bg-secondary border-3 transition duration-100 rounded-2xl h-60 sm:h-75 aspect-square flex flex-col justify-center items-center gap-1 cursor-pointer transform hover:-translate-y-1">
